...
while(locationAnswer !== 'yes' && locationAnswer !== 'y' && locationAnswer !== 'no' && locationAnswer !== 'n'){
  locationAnswer = prompt('Did I grow up in Missouri before moving to Florida in high school?').toLowerCase();
...
while(collegeAnswer !== 'yes' && collegeAnswer !== 'y' && collegeAnswer !== 'no' && collegeAnswer !== 'n'){
  collegeAnswer = prompt('Did I study Aerospace Engineering in college...even though I did not enjoy it as much as I enjoyed college football and Air Force ROTC?').toLowerCase();
...
while(awacsAnswer !== 'yes' && awacsAnswer !== 'y' && awacsAnswer !== 'no' && awacsAnswer !== 'n'){
  awacsAnswer = prompt('Did I join the Air Force and fly on the E-3 AWACS as an Air Battle Manager and deploy to the Middle East 3 times in support of operations against ISIS?').toLowerCase();
...
while(familyAnswer !== 'yes' && familyAnswer !== 'y' && familyAnswer !== 'no' && familyAnswer !== 'n'){
  familyAnswer = prompt('Is my wife\'s name Kayla and do we have 2 sons, Jacob (age 3) and Ethan (age 1)?').toLowerCase();
...
while(skydiveAnswer !== 'yes' && skydiveAnswer !== 'y' && skydiveAnswer !== 'no' && skydiveAnswer !== 'n'){
  skydiveAnswer = prompt('Did I propose to my wife while skydiving because that\'s how my parents met?').toLowerCase();
...
alert('I hope you learned a little more about me, ' + username + '!');
